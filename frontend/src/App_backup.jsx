...
function App() {
  ...
  useEffect(() => {
    ...
    if (courseSelect) {
      ...
      courseSelect.addEventListener('change', (event) => {
        ...
        const courseCode = selectedChoice ? selectedChoice.customProperties?.code : '';
        setFormData(prev => ({
          ...prev,
          course_title: event.detail.value,
  ...
  const renderPDFPreview = async (blob) => {
    try {
      ...
      // Validate blob
      if (!blob || blob.size === 0) {
        throw new Error('Empty or invalid PDF blob');
      }
      
      console.log('PDF blob size:', blob.size, 'bytes');
      
      // Check if it's actually a PDF - be more lenient with header check
      const arrayBuffer = await blob.arrayBuffer();
      const uint8Array = new Uint8Array(arrayBuffer);
      
      // Check first 10 bytes for debugging
      const first10Bytes = Array.from(uint8Array.slice(0, 10));
      const headerString = String.fromCharCode(...first10Bytes);
      console.log('PDF header bytes:', first10Bytes);
      console.log('PDF header string:', headerString);
      
      // More robust PDF check - look for %PDF anywhere in first 100 bytes
      const first100 = String.fromCharCode(...uint8Array.slice(0, 100));
      if (!first100.includes('%PDF-')) {
        console.error('No PDF header found in first 100 bytes:', first100);
        throw new Error('File is not a valid PDF format');
      }
      
      console.log('PDF header validation passed');
      
      // Use arrayBuffer directly for PDF.js
